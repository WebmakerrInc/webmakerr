...
class OrderController extends Controller
{
    ...
    public function store(OrderRequest $request)
    {
        ...
        if ($hasSubscription) {
            $type = 'subscription';
            // return with message right now we don't support subscription with manual order
        ...
        (new OrderCreated($order, $order->customer))->dispatch();
    ...
    private function updateOrderCustomer($customerId, $orderId)
    {
        ...
        fluent_cart_success_log(
            ...
            sprintf(__('Customer changed from %1$s to %2$s', 'fluent-cart'), $oldCustomer->full_name, $newCustomer->full_name),
            [
                'module_name' => 'order',
                'module_id'   => $orderId,
                'log_type'    => 'activity'
    ...
    public function deleteOrder(Request $request, $order_id)
    {
        ...
        if ($order->type === 'subscription') {
            ...
            return $this->sendSuccess([
                'message' => sprintf(__('Order %s deleted successfully', 'fluent-cart'), $order_id),
                'data'    => [
                    'order_id'   => $order_id,
                    'invoice_no' => $order->invoice_no,
                    'status'     => 'success'
...
//    }
    ...
    public function markAsPaid(Request $request, Order $order)
    {
        ...
        $actionActivity = [
            ...
            'content' => sprintf(__('Order status has been updated from %1$s to %2$s', 'fluent-cart'), $oldStatus, $order->status)
        ];
        ...
        if ($order->fulfillment_type == 'digital' && $order->status === Status::ORDER_PROCESSING) {
            ...
            $actionActivity = [
                'title'   => __('Order status updated', 'fluent-cart'),
                'content' => sprintf(__('Order status has been updated from %1$s to %2$s', 'fluent-cart'), Status::ORDER_PROCESSING, $order->status)
            ];
    ...
    public function handleBulkActions(Request $request)
    {
        ...
        if ($action == 'change_order_status') {
            ...
            if (count($failedOrderIds) > 0) {
                ...
                return count($updatedOrderIds) > 0
                    ? $this->sendSuccess([
                        'message' => sprintf(__("The order ID - %s cannot be updated because they are either already cancelled or have the same status. And remaining order status has been successfully changed", 'fluent-cart'), $failedOrderIds)
                    ])
                    :
                    $this->sendError([
                        'message' => sprintf(__("The order ID - %s cannot be updated because they are either already cancelled or have the same status.", 'fluent-cart'), $failedOrderIds)
                    ], 423);
            }
        ...
        if ($action == 'change_payment_status') {
            ...
            if (count($failedOrderIds) > 0) {
                ...
                return count($updatedOrderIds) > 0
                    ? $this->sendSuccess([
                        'message' => sprintf(__("The order ID - %s cannot be updated at the moment because the transaction either already has the same status or does not match the provided order. The remaining orders statuses have been updated successfully.", 'fluent-cart'), $failedOrderIds)
                    ])
                    :
                    $this->sendError([
                        'message' => sprintf(__("The order ID - %s cannot be updated at the moment because its payment status is either the same as before or has already been refunded.", 'fluent-cart'), $failedOrderIds)
                    ], 423);
            }

            if (count($updatedOrderIds) > 0 && count($failedOrderIds) < 1) {
                return $this->sendSuccess([
                    'message' => __("Selected orders payment status has been marked as %s", 'fluent-cart')
                ]);
            }
        }
