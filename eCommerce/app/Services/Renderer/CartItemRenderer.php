...
class CartItemRenderer
{
    ...
    public function render()
    {
        ...
        <div class="<?php $this->renderCssAtts($wrapperClassAttributes); ?>" role="listitem">
            ...
            <div class="fct_line_item_price" aria-label="<?php esc_attr_e('Price information'); ?>">
                <?php do_action('fluent_cart/cart/line_item/before_total', $this->getEventInfo()); ?>
                <?php if($promoPriceOriginal) : ?>
                <div style="text-decoration: line-through;" class="fct_line_item_total fct_promo_price" aria-label="<?php esc_attr_e('Original price'); ?>">
                    <?php echo Helper::toDecimal($promoPriceOriginal); ?>
                </div>
                <?php endif; ?>
                <span class="fct_line_item_total" aria-label="<?php esc_attr_e('Total price'); ?>">
    ...
    public function renderTitle()
    {
        ...
        <div class="fct_item_title">
            ...
            <?php if ($quantity > 1): ?>
                <span class="fct_item_quantity" aria-label="<?php echo esc_attr(sprintf(__('Quantity %d'), $quantity)); ?>">
            ...
            <?php if ($href): ?>
                <a
                   ...
                   aria-label="<?php echo esc_attr(sprintf(__('View details for %s'), $mainTitle)); ?>"
                >
            ...
            <?php if ($mainTitle != $subtitle && $subtitle): ?>
                <div class="fct_item_variant_title" aria-label="<?php esc_attr_e('Variant'); ?>">
    ...
    public function renderImage()
    {
        ...
        $href = Arr::get($this->item, 'view_url', '');
        $altText = sprintf(__('Image of %s', 'fluent-cart'), Arr::get($this->item, 'title', __('product', 'fluent-cart')));
        ?>
    ...
    protected function maybeRenderPaymentTypeInfo()
    {
        ...
        if ($paymentType === 'subscription') {
            ...
            <div class="fct_item_payment_info">
                <span class="sr-only"><?php esc_html_e('Payment information'); ?></span>
