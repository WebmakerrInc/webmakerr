...
class ReceiptRenderer
{
    ...
    public function wrapperStart()
    {
        ...
        <div class="fc-email-template-content email-template-content"
        style="font-family: Arial, Helvetica, sans-serif; margin: 0 auto; padding: 0px; width: 100%;">
        <div class="fc-email-template-content-inner" style="font-size: 13px;line-height: 1.4;color: #333;padding: 20px;">
    ...
    public function render($hideWrapper = false)
    {
        ...
        <style>
            ...
            .fc-transaction-table tr:last-child td {
    ...
    public function renderHeaderLogo()
    {
        ...
        <h1 style="font-size:24px;font-weight:700;color:rgb(17,24,39);margin:0px">
            ...
            if (empty($imageLink)) {
                ...
                echo "<img src='{$imageLink}' alt='{$storeName}' style='max-height: 50px;'>";
            }
            ?>
        </h1>
        <?php
    ...
    public function renderStoreTaxInformation()
    {
        ...
        if (!empty($storeVatNumber)): ?>
            <span class="fct_invoice_tax_content" style="font-weight: 500; font-size: 14px;">
                <?php echo TaxModule::getCountryTaxTitle($taxcountry); ?>:
                <?php
                if ($storeVatNumber !== '') {
    ...
    public function renderHeaderInvoiceNo()
    {
        ...
        ?>
        <p style="white-space: nowrap; color: #94a3b8; text-align: right; margin: 0;font-size:12px;">
            <?php echo __('Invoice number #', 'fluent-cart'); ?>
        </p>
        <p id="fc-order-invoice-no"
    ...
    <?php endif; ?>
        <div
            ...
            <?php echo esc_html($this->settings->get('store_address1')); ?>
        </div>

        <div class="fct-receipt-page-store-address-full-address">
            <?php echo esc_html(AddressHelper::getStateNameByCode(
                    $this->settings->get('store_state'),
                    $this->settings->get('store_country')
            )); ?>
            <?php echo esc_html(AddressHelper::getCountryNameByCode($this->settings->get('store_country'))); ?>
        </div>
        <?php
    }
    ...
    public function renderBillToAddress()
    {
        ...
        <div
            ...
            <h5
                    ...
                    style="font-weight: bold;font-size: 14px;margin: 0 0 10px 0;color: #495057;border-bottom: 1px solid #dee2e6;padding-bottom: 5px;">
                <?php echo __('Bill To', 'fluent-cart'); ?>
            </h5>
            <?php if (!empty($order->billing_address)) : ?>
                ...
                <div
                        ...
                        style="margin-bottom: 3px;">
                    <?php echo $order->billing_address->getAddressAsText(); ?>
                </div>
                <?php if (!empty($this->vat_tax_id)) : ?>
                    <div
                            class="fct-receipt-page-order-items-addresses-bill-to-vat-number"
                            style="margin-bottom: 3px;">
                        <?php echo __('VAT/Tax ID: ', 'fluent-cart') . esc_html($this->vat_tax_id); ?>
                    </div>
                <?php endif; ?>
                <div
                ...
                <div
                    ...
                    if ($vatNumber !== '') {
                        echo __('EU VAT', 'fluent-cart') . ': ' . esc_html($vatNumber);
                    }
                    ?>
                </div>
            ...
            <?php else: ?>
                ...
                <?php if (!empty($vat_tax_id)) : ?>
                    <div
                            class="fct-receipt-page-order-items-addresses-bill-to-vat-number"
                            style="margin-bottom: 3px;">
                        <?php echo __('VAT/Tax ID: ', 'fluent-cart') . esc_html($vat_tax_id); ?>
                    </div>
                <?php endif; ?>
                <div
                        class="fct-receipt-page-order-items-addresses-bill-to-email"
    ...
    public function renderShipToAddress()
    {
        ...
        <div
            ...
            <h5
                    ...
                    style="font-weight: bold;font-size: 14px;margin: 0 0 10px 0;color: #495057;border-bottom: 1px solid #dee2e6;padding-bottom: 5px;">
                <?php echo __('Ship To', 'fluent-cart'); ?>
            </h5>
            <?php if (!empty($order->shipping_address)) : ?>
                <div
                        class="fct-receipt-page-order-items-addresses-ship-to-address"
                        style="margin-bottom: 3px;">
                    <?php echo $order->shipping_address->getAddressAsText(); ?>
                </div>
                <?php
            ...
            <table
                ...
                <tr>
                    <th style="background-color: #f8f9fa;padding: 12px 8px;font-weight: bold;print-color-adjust: exact;border: none;">
                        <?php echo __('Description', 'fluent-cart'); ?>
                    </th>
                    <th style="background-color: #f8f9fa;padding: 12px 8px;font-weight: bold;width: 80px;text-align: right;print-color-adjust: exact;border: none;">
                        <?php echo __('Qty', 'fluent-cart'); ?>
                    </th>
                    <th style="background-color: #f8f9fa;padding: 12px 8px;font-weight: bold;width: 80px;text-align: right;print-color-adjust: exact;border: none;">
                        <?php echo __('Unit price', 'fluent-cart'); ?>
                    </th>
                    <th style="background-color: #f8f9fa;padding: 12px 8px;font-weight: bold;width: 80px;text-align: right;print-color-adjust: exact;border: none;">
                        <?php echo __('Amount', 'fluent-cart'); ?>
                    </th>
                </tr>
                </thead>
                <tbody>
                ...
                foreach ($orderItems as $item) :
                    ...
                    <tr>
                        ...
                        <td style="padding: 12px 8px;border: none;border-bottom: 1px solid #dee2e6;text-align: right">
                            <?php echo \FluentCart\Api\CurrencySettings::getFormattedPrice($item['unit_price']); ?>
                        </td>
                        <td style="padding: 12px 8px;border: none;border-bottom: 1px solid #dee2e6;text-align: right">
        ...
        if ($order->subtotal != ($order->total_amount - $order->total_refund) || $order->tax_total > 0): ?>
            <tr>
                <td style="padding: 8px 20px 8px 0;text-align: right;border: none;">
                    <?php echo __('Subtotal', 'fluent-cart'); ?>
                </td>
    ...
    public function renderDiscount()
    {
        ...
        if ($order->manual_discount_total + $order->coupon_discount_total > 0): ?>
            <tr>
                <td style="padding: 8px 20px 8px 0;text-align: right;border: none;">
                    <?php echo __('Discount', 'fluent-cart'); ?>
                </td>
    ...
    public function renderShipping()
    {
        ...
        if ($order->shipping_total > 0): ?>
            <tr>
                <td style="padding: 8px 20px 8px 0;text-align: right;border: none;">
                    <?php echo __('Shipping', 'fluent-cart'); ?>
                </td>
                <td style="padding: 8px 8px 8px 0;width: 100px;text-align: right;border: none;">
    ...
    public function renderShippingTax()
    {
        ...
        if ($order->shipping_tax > 0): ?>
            <tr>
                <td style="padding: 8px 20px 8px 0;text-align: right;border: none;">
                    <?php echo __('Shipping Tax', 'fluent-cart');
                    echo esc_html($order->tax_behavior == 2 ? __('(Included)', 'fluent-cart') : __('(Excluded)', 'fluent-cart'));
                    ?>
                </td>
                <td style="padding: 8px 8px 8px 0;width: 100px;text-align: right;border: none;">
        ...
        if ($order->total_refund > 0): ?>
            <tr style="font-weight: bold;font-size: 14px;">
                <td style="font-weight:500;padding: 8px 20px 8px 0;text-align: right;border:none;">
                    <?php echo __('Refund', 'fluent-cart'); ?>
                </td>
                <td style="padding: 8px 8px 8px 0;width: 100px;text-align: right;border:none;">
    ...
    public function renderTotal()
    {
        ...
        <tr style="font-weight: bold;font-size: 14px;">
            <td style="font-weight:500;padding: 8px 20px 8px 0;text-align: right;border:none;">
                <?php echo __('Total', 'fluent-cart'); ?>
            </td>
            <td style="padding: 8px 8px 8px 0;width: 100px;text-align: right;border:none;">
                <?php echo \FluentCart\Api\CurrencySettings::getFormattedPrice($order->total_amount - $order->total_refund); ?>
            </td>
        </tr>
        <?php
    }
    ...
    public function renderAmountPaid()
    {
        ...
        <tr style="font-weight: bold;font-size: 14px;">
            <td style="font-weight:500;padding: 8px 20px 8px 0;text-align: right;border:none;">
                <?php echo __('Amount Paid', 'fluent-cart'); ?>
            </td>
            <td style="padding: 8px 8px 8px 0;width: 100px;text-align: right;border:none;">
                <?php echo \FluentCart\Api\CurrencySettings::getFormattedPrice($order->total_paid - $order->total_refund); ?>
            </td>
        </tr>
        <?php
    }
    ...
    public function renderTaxNote()
    {
        ...
        if (Arr::get($orderTaxRates->meta ?? [], 'vat_reverse.valid') && !$taxtotal): ?>
            ...
            <div style="text-align: right; font-size: 14px; margin-top: 10px;">
                <?php echo '*' . esc_html__('Tax to be paid on reverse charge basis') ?>
            </div>
                    ...
                    <?php foreach ($transactions as $transaction) :
                        ...
                        <tr>
                            ...
                            <td style="padding: 10px;border:none;border-bottom: 1px solid #dee2e6;text-align: center;">
                                <?php echo DateTime::anyTimeToGmt($transaction->created_at)->format('M d, Y'); ?>
                            </td>
                            <td style="padding: 10px;border:none;border-bottom: 1px solid #dee2e6;text-align: right;">
                                <?php echo \FluentCart\Api\CurrencySettings::getFormattedPrice($transaction->total); ?>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                    </tbody>
    ...
    public function renderConfirmationError($errorData)
    {
        ...
        <div class="fluent_cart_confirmation_failed">
            ...
            <?php endif; ?>
            <span><?php echo esc_html('Please try to complete payment again from here!', 'fluent-cart'); ?></span>
            <a href="<?php echo esc_url($custom_payment_url ?? ''); ?>"><?php echo $custom_payment_url; ?></a>
        </div>
    <?php }
}
