...
class Stripe extends AbstractPaymentGateway
{
    ...
    public static function beforeSettingsUpdate($data, $oldSettings): array
    {
        ...
        foreach (['live', 'test'] as $credentialMode) {
            $secretKeyKey = $credentialMode . '_secret_key';
            $isEncryptedKey = $credentialMode . '_is_encrypted';
            $secretValue = Arr::get($data, $secretKeyKey);

            if ($secretValue && Helper::isValueEncrypted($secretValue)) {
                $data[$secretKeyKey] = Arr::get($oldSettings, $secretKeyKey, '');
                $data[$isEncryptedKey] = Arr::get($oldSettings, $isEncryptedKey, 'yes');
            } elseif ($secretValue) {
                $data[$secretKeyKey] = Helper::encryptKey($secretValue);
                $data[$isEncryptedKey] = 'yes';
            } else {
                $data[$secretKeyKey] = Arr::get($oldSettings, $secretKeyKey, '');
                $data[$isEncryptedKey] = Arr::get($oldSettings, $isEncryptedKey, 'no');
            }

            $webhookSecretKey = $credentialMode . '_webhook_secret';
            $webhookEncryptedKey = $credentialMode . '_webhook_is_encrypted';
            $webhookSecretValue = Arr::get($data, $webhookSecretKey);

            if ($webhookSecretValue && Helper::isValueEncrypted($webhookSecretValue)) {
                $data[$webhookSecretKey] = Arr::get($oldSettings, $webhookSecretKey, '');
                $data[$webhookEncryptedKey] = Arr::get($oldSettings, $webhookEncryptedKey, 'yes');
            } elseif ($webhookSecretValue) {
                $data[$webhookSecretKey] = Helper::encryptKey($webhookSecretValue);
                $data[$webhookEncryptedKey] = 'yes';
            } else {
                $data[$webhookSecretKey] = Arr::get($oldSettings, $webhookSecretKey, '');
                $data[$webhookEncryptedKey] = Arr::get($oldSettings, $webhookEncryptedKey, 'no');
            }

            $publishableKey = Arr::get($data, $credentialMode . '_publishable_key');
            if (($publishableKey === '' || $publishableKey === null) && Arr::get($oldSettings, $credentialMode . '_publishable_key')) {
                $data[$credentialMode . '_publishable_key'] = Arr::get($oldSettings, $credentialMode . '_publishable_key');
            }
        }
    ...
    public static function validateSettings($data): array
    {
        ...
        $provider = Arr::get($data, 'provider', 'api_keys');

        $settingsBase = new StripeSettingsBase();
        $existingSettings = $settingsBase->get();

        if ($provider === 'api_keys') {
            if ($mode === 'live') {
                $sk = defined('FCT_STRIPE_LIVE_SECRET_KEY') ? FCT_STRIPE_LIVE_SECRET_KEY : Arr::get($data, 'live_secret_key');
                $pk = defined('FCT_STRIPE_LIVE_PUBLIC_KEY') ? FCT_STRIPE_LIVE_PUBLIC_KEY : Arr::get($data, 'live_publishable_key');
            } else {
                $sk = defined('FCT_STRIPE_TEST_SECRET_KEY') ? FCT_STRIPE_TEST_SECRET_KEY : Arr::get($data, 'test_secret_key');
                $pk = defined('FCT_STRIPE_TEST_PUBLIC_KEY') ? FCT_STRIPE_TEST_PUBLIC_KEY : Arr::get($data, 'test_publishable_key');
            }

            if (!$pk) {
                $pk = Arr::get($existingSettings, $mode . '_publishable_key', '');
            }

            if (!$sk || Helper::isValueEncrypted($sk)) {
                $storedSecret = Arr::get($existingSettings, $mode . '_secret_key');
                if ($storedSecret) {
                    $decrypted = Helper::decryptKey($storedSecret);
                    $sk = $decrypted !== false ? $decrypted : $storedSecret;
                } else {
                    $sk = '';
                }
            }
        ...
        if (!$pk) {
            return [
                'status'  => 'failed',
                'message' => __('Please provide a valid publishable key!', 'fluent-cart')
            ];
        }

        if ($mode === 'live' && !str_contains($pk, 'pk_live')) {
            return [
                'status'  => 'failed',
                'message' => __('Please provide a valid LIVE publishable key!', 'fluent-cart')
            ];
        }

        if ($mode === 'test' && !str_contains($pk, 'pk_test')) {
            return [
                'status'  => 'failed',
                'message' => __('Please provide a valid TEST publishable key!', 'fluent-cart')
            ];
        }
    ...
    public function fields(): array
    {
        return array(
            ...
            'manual_setup'        => [
                'value' => '<div class="fc-connect-require"><h4>' . esc_html__('Configure Stripe manually', 'fluent-cart') . '</h4><p>' . esc_html__('Enter your Stripe API keys for live and test mode below. Save your changes to start accepting payments.', 'fluent-cart') . '</p></div>',
                'label' => __('Manual setup notice', 'fluent-cart'),
                'type'  => 'html_attr'
            ],
            'payment_mode'        => [
                'type'   => 'tabs',
                    ...
                    [
                        ...
                        'schema' => [
                            ...
                            'live_webhook_secret'  => [
                                ...
                                'type'        => 'password',
                                'placeholder' => __('whsec_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx', 'fluent-cart')
                            ]
                        ]
                    ],
                    [
    ...
    public function acceptRemoteDispute($transaction, $args = [])
    {
        ...
        if (is_wp_error($closeDispute)) {
            error_log($closeDispute->get_error_message());
            return $closeDispute;
        }
