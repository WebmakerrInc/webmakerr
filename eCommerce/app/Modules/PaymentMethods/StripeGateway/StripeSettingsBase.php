...
class StripeSettingsBase extends BaseGatewaySettings
{
    ...
    public function __construct()
    {
        ...
        if (!empty($settings['test_secret_key']) && Helper::isValueEncrypted($settings['test_secret_key'])) {
            $settings['test_is_encrypted'] = 'yes';
        }

        if (!empty($settings['live_secret_key']) && Helper::isValueEncrypted($settings['live_secret_key'])) {
            $settings['live_is_encrypted'] = 'yes';
        }

        if (!empty($settings['test_webhook_secret']) && Helper::isValueEncrypted($settings['test_webhook_secret'])) {
            $settings['test_webhook_is_encrypted'] = 'yes';
        }

        if (!empty($settings['live_webhook_secret']) && Helper::isValueEncrypted($settings['live_webhook_secret'])) {
            $settings['live_webhook_is_encrypted'] = 'yes';
        }
    ...
    public static function getDefaults()
    {
        return [
            ...
            'live_webhook_is_encrypted' => 'no',
            'test_webhook_is_encrypted' => 'no',
            'secure'               => 'yes'
        ];
    }
    ...
    public function getWebhookSecret($mode = 'current')
    {
        ...
        if ($mode === 'test') {
            $secret = $this->get()['test_webhook_secret'];
            if (!$secret) {
                return '';
            }

            $decrypted = Helper::decryptKey($secret);

            return $decrypted !== false ? $decrypted : $secret;
        }

        $secret = $this->get()['live_webhook_secret'];
