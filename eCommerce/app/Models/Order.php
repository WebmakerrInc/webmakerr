        ...
        if ($this->status === Status::ORDER_CANCELED) {
            ...
            if ($isPaidOrder && !$isFreeOrder) {
                $canBeDeleted = new \WP_Error(
                    ...
                    sprintf(
                        __('Order %s cannot be deleted due to its current payment status: %s.', 'fluent-cart'),
                        $this->invoice_no,
                        $this->payment_status
                    )
                );
            ...
            $canBeDeleted = new \WP_Error(
                'order_cannot_be_deleted',
                sprintf(
                    __('Order %s cannot be deleted due to its current order status: %s.', 'fluent-cart'),
                    $this->invoice_no,
                    $this->status
                )
            );
