...
{
    const SHORT_CODE = 'webmakerr_customer_profile';
    protected static string $shortCodeName = 'webmakerr_customer_profile';
    protected static array $aliases = ['fluent_cart_customer_profile'];
    ...
    public function render(?array $viewData = null)
    {
        if (!is_user_logged_in()) {
            ...
            if (defined('FLUENT_AUTH_VERSION') && (new \FluentAuth\App\Hooks\Handlers\CustomAuthHandler())->isEnabled()) {
                ...
                <h4><?php echo __('Please log in to access your customer portal.', 'fluent-cart'); ?></h4>
                <?php
                ...
                <div class="fct_auth_wrap">
                    <div class="fct_auth_message">
                        <h2><?php echo __('Login', 'fluent-cart'); ?></h2>
                        <p><?php echo __('Please log in to access your customer portal.', 'fluent-cart'); ?></p>
                        <a href="<?php echo esc_url(wp_login_url($redirectUrl ?? '')); ?>" class="button">
                            <?php echo __('Login', 'fluent-cart'); ?>
                        </a>
                    </div>
                </div>
                <?php
    ...
    public function maybeCustomEndpointContent() {
        ...
        if(isset($endpoint['page_id'])) {
            ...
            if ($page_query->have_posts()) :
                ...
                echo '<p>' . __('No content found!', 'fluent-cart') . '</p>';
            endif;
        }
