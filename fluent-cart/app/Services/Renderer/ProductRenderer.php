...
class ProductRenderer
{
    ...
    public function renderBuySection($atts = [])
    {
        ...
        echo '<div aria-labelledby="fc-product-summary-title" data-fluent-cart-product-pricing-section data-product-id="' . esc_attr($this->product->ID) . '" class="fct_buy_section">';
    ...
    public function renderGalleryThumb()
    {
        ...
        ?>
        <div class="fc-product-gallery-thumb" role="region" aria-label="<?php echo esc_attr($this->product->post_title . ' gallery'); ?>">
            <img
    ...
    public function renderTitle()
    {
        ...
        <div class="fc-product-title">
            <h1 id="fc-product-summary-title"><?php echo esc_html($this->product->post_title); ?></h1>
        </div>
        <?php
    }
    ...
    public function renderStockAvailability($wrapper_attributes = '')
    {
        ...
        echo sprintf(
                '<div class="fc-product-stock %1$s" role="status" aria-live="polite">
                    <div %2$s>
    ...
    public function renderExcerpt()
    {
        ...
        ?>
        <div class="fc-product-excerpt" aria-labelledby="fc-product-summary-title">
    ...
    public function renderPrices()
    {
        if ($this->product->detail->variation_type === 'simple') {
            ...
            ?>
            <div class="fc-price-range fc-product-prices" role="term" aria-label="<?php echo esc_attr(sprintf(__('Price: %s%s', 'fluent-cart'), $comparePrice ? Helper::toDecimal($comparePrice) . ' (original), ' : '', Helper::toDecimal($itemPrice))); ?>">
                <?php if ($comparePrice): ?>
                ...
                <?php endif; ?>
                <span class="fc-item-price" aria-label="<?php echo esc_attr(__('Current price', 'fluent-cart')); ?>">
        ...
        ?>
        <div class="fc-product-prices fc-price-range" role="term" aria-label="<?php echo esc_attr(sprintf(__('Price range: %s - %s', 'fluent-cart'), Helper::toDecimal($min_price), Helper::toDecimal($max_price))); ?>">
            <?php if ($max_price && $max_price != $min_price && $max_price > $min_price): ?>
                ...
                <span class="fc-price-separator" aria-hidden="true">-</span>
            <?php endif; ?>
    ...
    public function renderVariants($atts = [])
    {
        ...
        ?>
        <div class="<?php echo esc_attr(implode(' ', $classes)); ?>" role="radiogroup" aria-label="<?php esc_attr_e('Product Variants', 'fluent-cart'); ?>">
    ...
    public function renderItemPrice()
    {
        ...
        foreach ($this->product->variants as $variant): ?>
        <div
            class="fc-product-item-price fc-product-item-price-<?php echo esc_attr($variant->id); ?> <?php echo $this->defaultVariant->id != $variant->id ? ' is-hidden' : '' ?>"
            data-fluent-cart-product-item-price>
            <?php if ($this->defaultVariant && !$this->hasSubscription) {
                echo apply_filters('fluent_cart/single_product/variation_price', esc_html(Helper::toDecimal($variant->item_price)), [
                    'product' => $this->product,
                    'variant' => $variant,
                    'scope'   => 'product_variant_price'
                ]);
        ...
        foreach ($this->product->variants as $variant): ?>
            ...
            $atts = [
                'class' => 'fc-product-payment-type ' . (' fc-product-payment-type-'.$variant->id.' ') . ($paymentType !== 'subscription' || $this->defaultVariant->id != $variant->id ? ' is-hidden' : ''),
                'data-fluent-cart-product-payment-type' => ''
            ];
            ?>
            <div <?php $this->renderAttributes($atts); ?>>
                ...
                <?php endif; ?>
                <?php
                echo apply_filters('fluent_cart/single_product/variation_price', esc_html($variant->getSubscriptionTermsText(true)), [
                    'product' => $this->product,
                    'variant' => $variant,
                    'scope'   => 'product_variant_price'
                ]);
    ...
    public function renderQuantity()
    {
        ...
        <div <?php $this->renderAttributes($attributes); ?>>
            <label for="fc-product-qty-input" class="quantity-title">
        ...
        if (ModuleSettings::isActive('stock_management')) {
            if ($this->product->detail->variation_type === 'simple' && $this->defaultVariant) {
                if ($this->product->detail->manage_stock && $this->defaultVariant->stock_status !== Helper::IN_STOCK) {
                    echo '<span aria-disabled="true">' . __('Out of stock', 'fluent-cart') . '</span>';
                    return;
                }
            }
        }
        ...
        >
            ...
            <div class="variant-content">
                ...
                if ($this->viewType === 'both' || $this->viewType === 'text') {
                    echo '<div class="fc-product-variant-title" aria-label="' . esc_attr(__('Variant title', 'fluent-cart')) . '">' . esc_html($variant->variation_title) . '</div>';
                }
                ?>
            </div>
                ...
                <div class="fc-product-variant-price">
                    <?php if ($comparePrice): ?>
                        <div class="fc-product-variant-compare-price">
                            <del aria-label="<?php echo esc_attr(__('Original price', 'fluent-cart')); ?>"><span><?php echo esc_html(Helper::toDecimal($comparePrice)); ?></span></del>
                        </div>
                    <?php endif; ?>
                    <div class="fc-product-variant-item-price" aria-label="<?php echo esc_attr(__('Current price', 'fluent-cart')); ?>">
    ...
    {
        ?>
        <div class="fc-product-variant-tooltip" role="tooltip" id="tooltip-<?php echo esc_attr($variant->id); ?>">
    ...
    protected function renderVariantImage($variant)
    {
        ...
        <div class="fc-product-variant-image">
            <img role="img" alt="<?php echo esc_attr($variant->variation_title); ?>" src="<?php echo esc_url($image); ?>"/>
        </div>
        <?php
    }
    ...
    protected function renderSubscriptionInfo($variant)
    {
        ...
        ?>
        <div class="fc-product-variant-payment-type" aria-live="polite">
    ...
    protected function renderTabNav()
    {
        ...
        <div class="fc-product-tab-nav" role="tablist">
