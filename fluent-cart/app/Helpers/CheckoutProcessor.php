...
class CheckoutProcessor
{
    ...
    public function createDraftOrder($prevOrder = null)
    {
        ...
        if (!$customerId) {
            return new \WP_Error('customer_id_missing', __('Customer ID is required to create a draft order.', 'fluentcart'));
        }
        ...
        if (!$this->orderModel) {
            return new \WP_Error('order_creation_failed', __('Failed to create order.', 'fluentcart'));
        }
...
//            'total_amount'          => $this->orderTotals['total_amount'],
    ...
    private function convertToSubscriptionFormat($inputData)
    {
        ...
        $standardInterval = $intervalMap[$repeatInterval] ?? 'year';
        ...
        if ($totalDiscount > 0) {
            ...
            if ($firstCycleCost < $recurringAmount) {
                $adjustedTrialDays = $this->calculateTrialDaysForInterval($trialDays, $repeatInterval);
    ...
    private function calculateTrialDaysForInterval($trialDays, $repeatInterval)
    {

        switch ($repeatInterval) {
            case 'daily':
                return min($trialDays + 1, 365); // No change for daily
            case 'weekly':
                return min($trialDays + 7, 365); // Convert days to weeks
            case 'monthly':
                return min($trialDays + 30, 365); // Convert days to months
            case 'yearly':
                return min($trialDays + 365, 365); // Convert days to years
            default:
                return $trialDays; // Default case, no change
        }
    }
}
