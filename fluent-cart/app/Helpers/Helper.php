...
class Helper
{
    ...
    public static function getSubscriptionTermText(array $data, $asHtml = false): string
    {
        ...
        $unit       = $intervalOptions[$interval] ?? 'year';
        ...
        $pluralUnit = static function (string $unit, int $n): string {
            switch ($unit) {
                case 'day':   return _n('day', 'days',   $n, 'fluent-cart');
                case 'week':  return _n('week', 'weeks', $n, 'fluent-cart');
                case 'month': return _n('month', 'months', $n, 'fluent-cart');
                case 'year':  return _n('year', 'years', $n, 'fluent-cart');
                default:      return _n('month', 'months', $n, 'fluent-cart');
            }
        };

        // Build the “per …” phrase (e.g., "per month", "per 3 months")
        $perPhrase = ($count === 1)
            ? sprintf(__('per %s', 'fluent-cart'), $pluralUnit($unit, 1))
            : sprintf(__('per %1$d %2$s', 'fluent-cart'), $count, $pluralUnit($unit, $count));
    ...
    public static function generateSubscriptionInfo($otherInfo, $itemPrice): ?string
    {
        ...
        $interval = isset($intervalOptions[$repeatInterval])
            ? sprintf(__(' per %s', 'fluent-cart'), $intervalOptions[$repeatInterval])
            : '';

        $time = isset($intervalOptions[$repeatInterval])
            ? ($occurrence > 1 ? $intervalOptions[$repeatInterval] . 's' : $intervalOptions[$repeatInterval])
            : '';

        $paymentInfo = sprintf(__('%1$s %2$s, %3$s for %4$s %5$s', 'fluent-cart'), $price, $interval, __('until cancel', 'fluent-cart'), $occurrence, $time);

        if (empty($occurrence)) {
            $paymentInfo = sprintf(__('%1$s %2$s %3$s', 'fluent-cart'), $price, $interval, __('until cancel', 'fluent-cart'));
        }
     ...
     *
     * @param string $shortcode The shortcode to be inserted (e.g., '[webmakerr_receipt]').
    ...
    public static function generateDownloadFileLink($productDownload, $orderId = null, $validityInMinutes = 60): string
    {
