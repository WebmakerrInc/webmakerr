...
class ProductVariation extends Model
{
    ...
    public function canPurchase($quantity = 1)
    {
        if ($this->item_status !== 'active' ||
            ...
            return new \WP_Error('unpublished', __('This product is not available for purchase.', 'fluentcart'));
        }

        if ($this->payment_type === 'subscription' && $quantity > 1) {
            return new \WP_Error('invalid_subscription_quantity', __('You cannot purchase more than one subscription at a time.', 'fluentcart'));
        }
        ...
        if (ModuleSettings::isActive('stock_management')) {
            if (($productDetail->manage_stock && $this->manage_stock) && $quantity > $this->available) {
                return new \WP_Error('insufficient_stock', __('Insufficient stock available for this product.', 'fluentcart'));
            }
        }
