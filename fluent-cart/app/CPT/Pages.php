...
class Pages
{
    ...
    public function pages(): array
    {
        return [
            'checkout'         => [
                'title'   => 'Checkout',
                'content' => '[webmakerr_checkout]',
                'content_variants' => ['[webmakerr_checkout]', '[fluent_cart_checkout]']
            ],
            'cart'             => [
                'title'   => 'Cart',
                'content' => '[webmakerr_cart]',
                'content_variants' => ['[webmakerr_cart]', '[fluent_cart_cart]']
            ],
            'receipt'          => [
                'title'   => 'Receipt',
                'content' => '[webmakerr_receipt]',
                'content_variants' => ['[webmakerr_receipt]', '[fluent_cart_receipt]']
            ],
            'shop'             => [
                'title'   => 'Shop',
    ...
    public function getGeneratablePage(bool $withDefaultId = false): array
    {
        ...
        if ($withDefaultId) {
            foreach ($pages as $key => $page) {
                $title = $page['title'];
                $pageKey = "{$key}_page_id";
                $contentVariants = Arr::get($page, 'content_variants', [$page['content']]);
                $page = (new DynamicModel([], 'posts'))
                    ->newQuery()
                    ->where('post_type', 'page')
                    ->where('post_status', 'publish')
                    ->where(function ($query) use ($contentVariants) {
                        foreach ($contentVariants as $index => $contentVariant) {
                            $contentVariant = Str::of($contentVariant)->remove('/-->');
                            $likeQuery = "%" . $contentVariant . '%';
                            if ($index === 0) {
                                $query->where('post_content', 'LIKE', $likeQuery);
                            } else {
                                $query->orWhere('post_content', 'LIKE', $likeQuery);
                            }
                        }
                    })
                    ->orderByDesc('ID')
...
<div class="wp-block-fluent-cart-products"><div class="fc-products-wrapper" data-fluent-cart-shop-app="true" data-fluent-cart-product-wrapper=""><!-- wp:fluent-cart/shopapp-product-view-switcher {"className":"fluent-product-view-switcher","metadata":{"name":"View Switcher"}} /-->
...
<div class="fc-product-block-filter-item"><!-- wp:fluent-cart/shopapp-product-filter-apply-button /-->
...
<div class="fluent-cart-product-loop fc-product-block-editor-product-card"><!-- wp:fluent-cart/shopapp-product-image -->
...
<div class="fc-product-block-filter-item"><!-- wp:paragraph {"align":"center","fontSize":"large"} -->
