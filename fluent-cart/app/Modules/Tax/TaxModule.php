...
class TaxModule
{
    ...
    public function handleVatValidation()
    {
        ...
        $vatNumber = isset($_REQUEST['vat_number']) ? sanitize_text_field($_REQUEST['vat_number']) : '';
    ...
    public function removeVat()
    {
        ...
        if (isset($_REQUEST['fct_cart_hash'])) {
            $cart = CartResource::get(['hash' => sanitize_text_field($_REQUEST['fct_cart_hash'])]);
        } else {
    ...
    protected function validateEuVatNumber($countryCode, $vatNumber)
    {
        try {
            ...
            if (empty($result->valid)) {
                throw new \Exception(__('Invalid VAT number for country ' . $countryCode . '!', 'fluent-cart'));
            }
