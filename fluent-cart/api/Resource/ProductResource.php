    ...
    public static function create($data, $params = [])
    {
        ...
        if (!$createdPostId) {
            return static::makeErrorResponse([
                ['code' => 403, 'message' => __($createdPostId->get_error_message(), 'fluent-cart')]
            ]);
        }
        ...
        if (!empty($product)) {
            ...
            if ($deletedProduct) {
                ...
                fluent_cart_success_log(
                    ...
                    sprintf(__('Product %s is deleted', 'fluent-cart'), $productTitle),
                    [
                        'module_name' => 'Product',
                        'module_id'   => 0,
        ...
        if ($action == 'delete_products') {
            ...
            if (count($failedProductIds) > 0) {
                ...
                return count($deletedProductIds) > 0
                    ? static::makeSuccessResponse('', sprintf(__("The Product ID - %s cannot be deleted at the moment as there are pending orders associated with it. And remaining product and its associated data have been deleted", 'fluent-cart'), $failedProductIds))
                    : static::makeErrorResponse([['code' => 400, 'message' => sprintf(__("The Product ID - %s cannot be deleted at the moment as there are pending orders associated with it.", 'fluent-cart'), $failedProductIds)]]);
            }
