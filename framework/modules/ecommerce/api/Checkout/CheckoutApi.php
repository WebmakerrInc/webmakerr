...
class CheckoutApi
{
    ...
    private static function finalizeOrder(Order $order, $args = [])
    {
        ...
        (new OrderCreated($order, $order->customer, $order->getLatestTransaction()))->dispatch();
    ...
    public static function validateData($data, Cart $cart, CheckoutService $cartCheckoutService, $prevOrder)
    {
        ...
        foreach ($billingValidations as $key => $rule) {
            ...
            if ($key === 'country') {
                ...
                if (!Arr::has($countries, $value)) {
                    ...
                    $errors[$prefixedKey]['invalid'] = sprintf(__('%s is invalid.', 'fluent-cart'), $titledKey);
                }
            }
            ...
            if ($key === 'state') {
                ...
                if (!empty($states) && !in_array($value, array_column($states, 'value'))) {
                    if (!isset($errors[$key])) {
                        $errors[$prefixedKey] = [];
                    }
                    $errors[$prefixedKey]['invalid'] = sprintf(__('%s is invalid.', 'fluent-cart'), $titledKey);
            ...
            if ($rule === 'required' && empty($value)) {
                ...
                $errors[$prefixedKey]['required'] = sprintf(__('%s is required.', 'fluent-cart'), $titledKey);
            }
        }
        ...
        foreach ($basicInfoFields as $field) {
            ...
            if ($fieldName && $isRequired) {
                ...
                if (empty($value)) {
                    Arr::set($errors, $fieldName . '.required', sprintf(__('%s is required.', 'fluent-cart'), Arr::get($field, 'aria-label')));
                }
            }
        }
