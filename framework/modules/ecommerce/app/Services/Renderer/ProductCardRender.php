...
class ProductCardRender
{
    ...
    public function renderExcerpt($atts = '')
    {
        ...
        echo sprintf(
            '<p %1$s class="fc-product-card-excerpt">
                   %2$s
            </p>',
            $atts,
            esc_html($this->product->post_excerpt),
        );
    }

    public function renderTitle($atts = '', $config = [])
    {
        $link = Arr::get($config, 'isLink');
        ...
        if ($link) {
            ...
            echo sprintf(
                '<h3 class="fc-product-card-title">
            ...
            echo sprintf(
                '<h3 class="fc-product-card-title" %1$s>%2$s</h3>',
                $atts,
                $titleText
            );
        }
    ...
    public function showBuyButton($atts = '')
    {
        ...
        if ($isInstantCheckout && !empty($customAttributes)) {
