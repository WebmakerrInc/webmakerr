...
class StatusHelper
{
    ...
    public function changeOrderStatus($orderStatus, $paymentStatus, $title, $slug)
    {
        ...
        $actionActivity = [
            ...
            'content' => sprintf(__('Order status has been updated from %s to %s', 'fluent-cart'), $oldStatus, $orderStatus)
        ];
    ...
    public function syncOrderStatuses($latestTransaction = null)
    {
        ...
        if ($oldOrderStatus != $this->order->status) {
            $actionActivity = [
                'title'   => __('Order status updated', 'fluent-cart'),
                'content' => sprintf(__('Order status has been updated from %1$s to %2$s', 'fluent-cart'), $oldOrderStatus, $this->order->status)
            ];
