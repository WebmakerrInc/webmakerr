...
class Discount_Code_List_Table extends Base_List_Table {
	...
	public function __construct() {
		...
		parent::__construct(
			[
				'singular' => __('Discount Code', 'ultimate-multisite'),  // singular name of the listed records
				'plural'   => __('Discount Codes', 'ultimate-multisite'), // plural name of the listed records
	...
	public function column_name($item): string {
		...
		$actions = [
			'edit'   => sprintf('<a href="%s">%s</a>', wu_network_admin_url('wp-ultimo-edit-discount-code', $url_atts), __('Edit', 'ultimate-multisite')),
			'delete' => sprintf(
	...
	public function column_value($item) {
		...
		if ( ! $item->get_value()) {
			return __('No Discount', 'ultimate-multisite');
		}
		...
		return sprintf(__('%s OFF', 'ultimate-multisite'), $value);
	}
	...
	public function column_uses($item) {
		...
		$html = sprintf(__('Used %d time(s)', 'ultimate-multisite'), $item->get_uses());
		...
		if ($item->get_max_uses() > 0) {
			...
			$html .= '<small class="wu-block">' . sprintf(__('Allowed uses: %d', 'ultimate-multisite'), $item->get_max_uses()) . '</span>';
		} else {
			$html .= '<small class="wu-block">' . __('No Limits', 'ultimate-multisite') . '</span>';
		}
	...
	public function column_coupon_code($item) {
		...
		if (is_wp_error($valid)) {
			$html .= sprintf('<small class="wu-block wu-sans" role="tooltip" aria-label="%s">%s</small>', esc_attr($valid->get_error_message()), esc_html__('Inactive', 'ultimate-multisite'));
		}
	...
	public function get_columns() {
		...
		$columns = [
			...
			'coupon_code'     => __('Code', 'ultimate-multisite'),
			'uses'            => __('Uses', 'ultimate-multisite'),
			'value'           => __('Value', 'ultimate-multisite'),
			'setup_fee_value' => __('Setup Fee Value', 'ultimate-multisite'),
			'date_expiration' => __('Dates', 'ultimate-multisite'),
		];
